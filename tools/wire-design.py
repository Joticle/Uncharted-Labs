...
LIVE_JS = r"""
  ...
  mapLive(dec, pf, stageIndex) {
    ...
    const spreadsFromBroker = () => {
      ...
      const out = [];
      groups.forEach((legs) => {
        const lng = legs.find((l) => l.qty > 0);
        const sht = legs.find((l) => l.qty < 0);
        if (!lng || !sht) return;

        const n = Math.abs(lng.qty);
        const shares = n * 100;
        const debitPerShare = (lng.costBasis + sht.costBasis) / shares;
        const unrealised = legs.reduce((a, l) => a + l.unrealizedPl, 0);
        const markPerShare = (lng.marketValue + sht.marketValue) / shares;
        const dte = Math.max(0, Math.round((Date.parse(lng.expiry + 'T20:00:00Z') - Date.now()) / 86400000));
        const px = (l) => (Math.abs(l.costBasis) / (Math.abs(l.qty) * 100)).toFixed(2);

        out.push({
          title: lng.underlying + ' ' + lng.strike + 'C / ' + sht.strike + 'C',
          sym: lng.underlying,
          kind: 'call debit',
          qty: n + ' ×',
          dte: dte + ' DTE',
          ...
          legs: [
            { side: 'LONG', strike: lng.strike + 'C', px: px(lng), dist: '' },
            { side: 'SHORT', strike: sht.strike + 'C', px: px(sht), dist: '' },
          ],
          metrics: [
            ['net debit', debitPerShare.toFixed(2)],
            ['mark', markPerShare.toFixed(2)],
            ['max gain', '$' + Math.round((sht.strike - lng.strike - debitPerShare) * shares).toLocaleString('en-US')],
          ],
        });
      });
      return out;
